...
async def _auto_fetch_loop():
    while True:
        try:
            ...
            dates_to_fetch = [today, yesterday]
            logger.info(f"Auto-fetch: starting for {dates_to_fetch}")
            ...
            for target_date in dates_to_fetch:
                # 1. 抓取 HF 论文
                try:
                    hf_papers = await fetch_daily_papers(target_date)
                    if hf_papers:
                ...
                for profile in profiles:
                    ...
                    for cat in [c.strip() for c in cats.split(",") if c.strip()]:
                        if cat not in fetched_cats:
                            ...
                            try:
                                papers = await fetch_arxiv_papers(target_date, cat)
                                if papers:
                                    inserted = await insert_arxiv_papers(target_date, cat, papers)
                                    if inserted > 0:
                                        logger.info(f"Auto-fetch: ArXiv {cat} {target_date} inserted {inserted} papers")
                            except Exception as e:
                                logger.error(f"Auto-fetch ArXiv {cat} {target_date} error: {e}")

                # 为未生成 brief 的 arxiv 论文生成概要
                all_arxiv = await get_arxiv_papers_by_date(target_date)
                ...
                if analyze_profiles:
                    hf_need = await get_hf_papers_need_detail(target_date)
                    arxiv_need = await get_arxiv_papers_need_detail(target_date)
